...
const App: React.FC = () => {
  ...
  // Guardar en localStorage cada vez que cambie un estado
  ...
  const renderView = () => {
    switch (activeView) {
      ...
      case 'projects': return <ProjectsTP projects={projects} setProjects={setProjects} searchTerm={searchTerm} />;
      case 'costs': return <CostsView advances={advances} />;
      case 'orders': return <OrdersView searchTerm={searchTerm} orders={orders} setOrders={setOrders} />;
      ...
      <main className="flex-1 ml-64 p-8 min-h-screen">
        <div className="flex justify-between items-center mb-8 bg-white p-4 rounded-xl border border-slate-200 shadow-sm sticky top-8 z-40">
          <div className="relative w-96">
            ...
            <input 
              type="text" 
              placeholder="Buscar por módulo, entidad o pedido..." 
              value={searchTerm}
          ...
          <div className="flex items-center gap-4">
            <div className="text-right hidden md:block">
              <p className="text-sm font-bold text-slate-800 leading-none">Administrador Operativo</p>
              <p className="text-[10px] text-amber-600 font-black mt-1 uppercase tracking-widest">Control Central</p>
            </div>
            <div className="w-10 h-10 rounded-full bg-slate-900 flex items-center justify-center border-2 border-amber-500 shadow-lg">
